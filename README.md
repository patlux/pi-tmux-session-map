...
On `session_start`, `session_info_changed`, and `agent_end`, the extension writes the active session file path to:

```
~/.local/state/pi/tmux-sessions/<session_name>_<window_index>.<pane_index>.session
```

The key `#{session_name}:#{window_index}.#{pane_index}` is stable across tmux server restarts (unlike pane ids like `%42`), which is exactly what tmux-resurrect restores. Characters outside `[A-Za-z0-9._-]` are replaced with `_`.
...
The mapping is written only when:
